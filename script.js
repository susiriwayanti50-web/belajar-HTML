...
// GAMBAR FALLBACK / DEFAULT JIKA LINK DIRETAK ATAU ERROR
const GAMBAR_DEFAULT = "https://via.placeholder.com/160?text=Gambar+Produk";
...
function simpanBarang(){
  ...
  let gambar = document.getElementById('gambarBarang').value.trim() || GAMBAR_DEFAULT;
...
function loadStokAdmin(){
  let html = '';
  produk.forEach(p => {
    let imgUrl = p.gambar || GAMBAR_DEFAULT;
    html += `<tr>
      <td><img src="${imgUrl}" onerror="this.onerror=null;this.src='${GAMBAR_DEFAULT}';" style="width:40px; height:40px; object-fit:cover; border-radius:4px;"></td>
      <td><b>${p.nama}</b></td>
      <td>${p.kategori}</td>
  ...
  produk.forEach(p => {
    ...
    <div class="produk-card">
        <div class="produk-img">
            ...
            <img src="${imgUrl}" onerror="this.onerror=null;this.src='${GAMBAR_DEFAULT}';" alt="${p.nama}">
        </div>
