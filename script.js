// URL Video Default Baru
const DEFAULT_DEMO_VIDEO = "https://vjs.zencdn.net/v/oceans.mp4";
...
function openProductModal(name, price, desc, imgSrc, videoSrc) {
    ...
    if (videoSrc) {
        ...
        modalVid.src = DEFAULT_DEMO_VIDEO;
    }
...
document.getElementById('addProductForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        // Membaca file gambar dan video secara asinkron
        const imgSrc = await readFileAsDataURL(imgFile);
        let videoSrc = await readFileAsDataURL(videoFile);
        
        if (!videoSrc) {
            videoSrc = DEFAULT_DEMO_VIDEO; // Default demo video baru jika tidak mengunggah video
        }
        ...
        // Render Kartu Produk Baru dengan Harga Terformat
        card.innerHTML = `
        ...
        card.querySelector('.product-clickable').addEventListener('click', function() {
            openProductModal(name, formattedPrice, desc, imgSrc, videoSrc);
        });
