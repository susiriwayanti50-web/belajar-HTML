...
function login(){
  ...
  if(u.toLowerCase() === "suci" && p === "12345"){
    ...
    // Siapa saja bisa login sebagai pembeli
    userLogin = u;
    localStorage.setItem('userLogin', u);
    document.getElementById('loginBox').style.display = 'none';
    document.getElementById('pelangganDashboard').style.display = 'block';
...
// UPDATE LAPORAN PENJUALAN ADMIN (PILIH STATUS PENGIRIMAN)
...
function showPelangganPage(pageId){
  ...
  if(pageId === 'pengirimanPage') loadPesananPelanggan();
  if(pageId === 'laporanPelangganPage') loadPesananPelanggan();
}
...
function prosesKonfirmasiCheckout() {
  ...
  itemCheckoutSementara.forEach(k => {
    ...
    pesanan.push({
      ...
      jumlah: k.jumlah,
      total: k.harga * k.jumlah,
      status: 'Sedang Dikemas' // Status awal saat checkout
    });
  });
...
function loadPesananPelanggan(){
  ...
  myOrders.forEach(p => {
    ...
    let tombolTerima = p.status !== 'Sudah Diterima' 
      ? `<button class="btn-beli" style="padding:4px 8px; font-size:10px;" onclick="terimaBarang(${p.id})">Terima Barang</button>`
